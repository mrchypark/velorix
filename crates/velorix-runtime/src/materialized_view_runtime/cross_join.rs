//! CROSS JOIN runtime (Phase 8.3).
//!
//! Materializes `SELECT ... FROM left CROSS JOIN right` for two registered
//! relations. Every epoch applies both sides atomically and recomputes the
//! full pair set from the post-epoch states, then diffs against the
//! previous output (exact, same class as the interval join full-output
//! diff). The projection must include both primary keys so output rows are
//! unique per pair (bag semantics preserved); the resource contract bounds
//! state and per-epoch output and fails the epoch closed on overflow.
...
pub struct CrossJoinRuntime {
    ...
    left_rows: BTreeMap<String, CrossJoinRow>,
    right_rows: BTreeMap<String, CrossJoinRow>,
...
impl CrossJoinRuntime {
    pub fn new_with_logical_plan(
        ...
        validate_logical_view_plan(&logical_plan).map_err(|_| {
            StandingProgramRuntimeError::InvalidProgramIdentity {
                field: "logical_cross_join_plan",
            }
        })?;
        validate_cross_join_contract(&catalogs, &input_schemas, &plan)?;
        let compiled =
            validate_supported_cross_join_sql(view_sql.as_str(), &catalogs).map_err(|_| {
                StandingProgramRuntimeError::InvalidProgramIdentity {
                    field: "cross_join_plan",
                }
            })?;
        if compiled != plan {
        ...
        let (left_input_schema, right_input_schema) = match input_schemas.as_slice() {
            ...
            _ => {
                return Err(StandingProgramRuntimeError::InvalidProgramIdentity {
                    field: "cross_join_input_schemas",
                })
            }
        };
        Ok(Self {
    ...
    fn apply_side(
        rows: &mut BTreeMap<String, CrossJoinRow>,
        record: &DeltaRecord,
    ) -> Result<(), StandingProgramRuntimeError> {
        let key = canonical_json(record.key.as_json());
        let entry = rows.entry(key.clone()).or_insert_with(|| CrossJoinRow {
            values: BTreeMap::new(),
            weight: 0,
        });
        entry.weight = entry
    ...
    fn recompute_all_pairs(&self) -> Result<DeltaBatch, StandingProgramRuntimeError> {
        ...
        {
            return Err(StandingProgramRuntimeError::InvalidProgramIdentity {
                field: "cross_join_resource_contract",
            });
        }
        ...
        for left in self.left_rows.values() {
            for right in self.right_rows.values() {
                ...
                for item in &self.plan.projection {
                    ...
                    output.insert(item.output_name.clone(), value.clone());
                }
...
impl StandingProgramRuntime for CrossJoinRuntime {
        ...
        for input in &input_changes {
            let (side, schema, catalog, rows) =
                if input.relation_id == self.plan.left_input_relation_id {
                    ...
                    return Err(StandingProgramRuntimeError::InvalidProgramIdentity {
                        field: "cross_join_input_relation",
                    });
                };
            validate_input_matches_schema(input, schema, "cross_join_input")?;
    ...
    fn checkpoint(&self) -> Result<RuntimeCheckpoint, StandingProgramRuntimeError> {
        let payload = CrossJoinCheckpointPayloadV2 {
            schema_version: CHECKPOINT_PAYLOAD_SCHEMA_VERSION,
            runtime_kind: CROSS_JOIN_RUNTIME_KIND.to_string(),
            ...
            right_rows: self.right_rows.clone(),
    ...
    fn restore(checkpoint: RuntimeCheckpoint) -> Result<Self, StandingProgramRuntimeError> {
        ...
        if payload.schema_version != CHECKPOINT_PAYLOAD_SCHEMA_VERSION
            || payload.runtime_kind != CROSS_JOIN_RUNTIME_KIND
        {
            return Err(invalid_checkpoint());
        }
        validate_cross_join_contract(
        ...
        let compiled = validate_supported_cross_join_sql(
...
#[derive(Clone, Debug, Deserialize, Serialize)]
#[serde(deny_unknown_fields)]
struct CrossJoinCheckpointPayloadV2 {
    schema_version: u32,
    runtime_kind: String,
    left_catalog: VelorixRelationCatalogV1,
    right_catalog: VelorixRelationCatalogV1,
...
pub(super) const CROSS_JOIN_RUNTIME_KIND: &str = "cross_join_v2";

fn validate_cross_join_contract(
    ...
    for item in &plan.projection {
        ...
        catalog_column(catalog, &item.column_id)?;
        if !output_names.insert(item.output_name.clone()) {
            return Err(StandingProgramRuntimeError::InvalidProgramIdentity {
                field: "cross_join_contract",
            });
        }
