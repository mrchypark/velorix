...
pub use velorix_control::runtime_contract::MATERIALIZED_VIEW_RUNTIME_NAME as CRATE_NAME;
...
impl StandingProgramRuntime for LatestByKeyRuntime {
    ...
    fn apply_changes(
        ...
        let mut next_latest_state = self.latest_state.clone();
        let mut executor = LogicalPlanExecutor::LatestByKey {
            catalog: &self.catalog,
            input_schema: &self.input_schema,
            plan: &self.plan,
            latest_state: &mut next_latest_state,
        ...
        let (next_published_output, output_delta) = if self.plan.top_k.is_some() {
            let previous_output = self.published_output.clone();
            let next_published_output = apply_latest_top_k_to_published_output(
                next_latest_state.materialized_delta(&self.plan),
                self.plan.top_k.as_ref(),
                &self.plan,
            )?;
            let output_delta = previous_output
                .inverse()
                .map_err(|_| invalid_runtime_state())?
                .combine(&next_published_output);
            (next_published_output, output_delta)
        } else {
            let next_published_output = apply_published_output_delta(
                &self.published_output,
                &executor_commit.output_delta,
            )?;
            (next_published_output, executor_commit.output_delta)
        };
        self.latest_state = next_latest_state;
        self.published_output = next_published_output;
...
impl StandingProgramRuntime for SingleKeySumCountRuntime {
    ...
    fn apply_changes(
        ...
        if single_key_plan_uses_runtime_aggregate_state(&self.plan) {
            ...
            let mut next_engine = self.engine.clone();
            next_engine
        ...
        let output_delta = if self.plan.top_k.is_some() {
            ...
            let full_output = filter_output_delta_for_having(
                &next_engine.materialized_state(),
                self.plan.having.as_ref(),
                self.plan.having_expr.as_ref(),
                &self.output_schema,
                Some(&aggregate_outputs),
...
impl StandingProgramRuntime for TwoInputJoinRuntime {
    ...
    fn apply_changes(
        ...
        if join_plan_uses_runtime_aggregate_state(&self.plan) {
            let mut next_join = self.join.clone();
            let mut next_engine = self.engine.clone();
            let mut joined_changes = DeltaBatch::default();
            let mut input_frontiers = self.input_frontiers.clone();
            let mut input_event_time_frontiers = self.input_event_time_frontiers.clone();
        ...
        let filtered_output_delta = filter_output_delta_for_having(
            &executor_commit.output_delta,
            self.plan.having.as_ref(),
            self.plan.having_expr.as_ref(),
            &self.output_schema,
        ...
        let (output_delta, next_published_output) = if self.plan.top_k.is_some() {
            let previous_output = self.published_output.clone();
            let aggregate_outputs = supported_join_view_plan_aggregate_outputs(&self.plan);
            let full_output = filter_output_delta_for_having(
                &next_engine.materialized_state(),
            ...
            let next_published_output = apply_top_k_to_published_output(
                full_output,
                self.plan.top_k.as_ref(),
                &aggregate_outputs,
            )?;
            let output_delta = previous_output
                ...
                .map_err(|_| invalid_runtime_state())?
                .combine(&next_published_output);
            (output_delta, next_published_output)
        } else {
            let next_published_output =
                apply_published_output_delta(&self.published_output, &filtered_output_delta)?;
            (filtered_output_delta, next_published_output)
        };
        self.join = next_join;
        self.engine = next_engine;
        self.published_output = next_published_output;
