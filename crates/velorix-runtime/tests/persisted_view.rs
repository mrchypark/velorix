...
use object_store::{
    local::LocalFileSystem, path::Path, CopyOptions, ObjectStore,
    ObjectStoreExt as AuthorityObjectStoreExt, PutMode,
};
...
async fn persisted_object_backed_view_propagates_datafusion_scan_errors() {
    ...
    assert!(matches!(
        error,
        PersistedViewError::RuntimeQuery(RuntimeQueryError::Query(QueryError::Engine(_)))
    ));
}
