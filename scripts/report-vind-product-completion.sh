...
python3 - "$product_evidence" "$rest_api_smoke_evidence" "$output_file" "$product_dir" <<'PY'
import json
import os
import sys
from datetime import datetime, timezone

product_path, rest_smoke_path, output_path, product_dir = sys.argv[1:]
...
product_complete = all(item["status"] in {"pass", "out_of_scope"} for item in gates)
...
blockers = [
    gate_blocker(item)
    for item in gates
    if item.get("status") in {"blocked", "diagnostic", "missing"}
]
...
completion_steps = [
    ...
    for item in gates
    if item.get("status") not in {"pass", "out_of_scope"}
]
...
payload = {
    ...
    "product_completion_source": {
        ...
        "accepted_gate_statuses": ["pass", "out_of_scope"],
        "all_required_gates_pass_or_out_of_scope": product_complete,
    },
    "completion_scope": {
        "external_s3_required": external_s3_required,
        "public_ingress_required": public_ingress_required,
