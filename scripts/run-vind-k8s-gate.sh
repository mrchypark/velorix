...
evidence = {
    ...
    "readiness_evidence_kind": [
        ...
        "kubernetes_worker_shard_periodic_resync_requeue",
        "kubernetes_worker_shard_live_leader_handoff",
    ...
    "limitations": [
        ...
        "worker-shard startup and bounded periodic resync run against local vind Kubernetes but do not prove production multi-pod controller lifecycle management",
        "worker-shard owner handoff is exercised across two independently assembled runtimes against local vind Kubernetes but does not prove production multi-pod leader election",
        "worker-shard stale Pod replacement is exercised after release/reacquire against local vind Kubernetes but does not prove production S3-compatible authority behavior",
