...
rg -q '^  local-nightly:$' "$workflow_path"
rg -q 'cargo test --workspace' "$workflow_path"
rg -q 'cargo bench -p velorix-runtime --bench local_incremental' "$workflow_path"
rg -q -- '--backend local' "$workflow_path"
rg -q -- '--baseline baselines/benchmark/local/pr-smoke\.json' "$workflow_path"
if awk '
...
rg -q '^      AWS_ACCESS_KEY_ID: \$\{\{ secrets\.AWS_ACCESS_KEY_ID \}\}$' "$workflow_path"
rg -q '^      AWS_SECRET_ACCESS_KEY: \$\{\{ secrets\.AWS_SECRET_ACCESS_KEY \}\}$' "$workflow_path"
if rg -ni 'gcloud|cloud[- ]build|google-github-actions' "$workflow_path"; then
...
rg -q 'echo "s3_gate_skipped=true"' "$workflow_path"
rg -q '^      - name: Mark optional S3-compatible gate skipped$' "$workflow_path"
rg -q "if: steps.config.outputs.s3_gate_skipped == 'true'" "$workflow_path"
rg -q 'echo "live_s3_skipped=true"' "$workflow_path"
rg -q '^      - name: Mark live S3-compatible tests skipped$' "$workflow_path"
if rg -q 'nightly gate requires S3_BENCHMARK_RESULT_PATH or explicit live S3-compatible test opt-in' "$workflow_path"; then
...
rg -q 'scripts/validate-nightly-output\.sh benchmark_result_path "\$S3_BENCHMARK_RESULT_PATH"' "$workflow_path"
rg -q 'BENCHMARK_RESULT_PATH: \$\{\{ steps.config.outputs.benchmark_result_path \}\}' "$workflow_path"
rg -q 'test -f "\$BENCHMARK_RESULT_PATH"' "$workflow_path"
rg -q -- '--result "\$BENCHMARK_RESULT_PATH"' "$workflow_path"
if awk '
    /^        run: \|$/ { in_run = 1; next }
    in_run && /^        [^ ]/ { in_run = 0 }
    in_run && /\$\{\{/ { found = 1 }
...
rg -q 'newline or carriage return' "$output_validator"
rg -q 'NUL or control character' "$output_validator"
rg -q 'for name in AWS_ENDPOINT_URL AWS_ACCESS_KEY_ID AWS_SECRET_ACCESS_KEY AWS_REGION VELORIX_S3_BUCKET;' "$workflow_path"
rg -q 'live S3-compatible tests requested but missing required env:' "$workflow_path"
rg -q "if: steps.config.outputs.live_s3_configured == 'true'" "$workflow_path"
